					...
					<v-row>
						...
						<v-col cols="12" md="4" >
							<div class="pa-5 d-flex flex-column justify-content-between">
								...
								<div>
									<v-img :src="currentObject.eight_ball ? getUrl(currentObject.eight_ball) : getImageDefaultPlaceholder()" :alt="currentObject.eight_ball_alt"></v-img>
									<ImagePicker @updateDeletedPhoto="updateDeletedPhoto" :activePhotoPath="currentObject.eight_ball" @loadedImage="$event == 'placeholder' ? currentObject.eight_ball = '' : currentObject.eight_ball = $event" :img="currentObject.eight_ball"/>
								</div>
								
								<v-text-field color="primary"  v-model="currentObject.eight_ball_alt" label="Tekst alternatywny zdjęcia"></v-text-field>
							</div>
						</v-col>
