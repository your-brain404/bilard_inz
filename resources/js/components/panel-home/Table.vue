<template>
	<v-card shaped class="">
		<v-card-title class="align-items-center pt-5">
			...
			<router-link v-if="block.multiple " class="form-link" :to="`/admin-panel/${block.tablename}/${block.parent ? `${$route.params.parent_id}/` : ''}form`">
				<v-btn class="first-bgc white--text ml-5">
					<v-icon left class="">{{  block.add_button_icon ? block.add_button_icon : 'mdi-plus' }}</v-icon>
					<span>{{ block.add_button_text ? block.add_button_text : 'Dodaj' }}</span>
				</v-btn>
			</router-link>
		</v-card-title>
		<v-data-table locale="pl" :headers="headers[i]" :items="block.table" :search="search" :items-per-page="5" :footer-props="{ 'items-per-page-options': [5,10,15] }" class="elevation-1" @page-count="pageCount = $event" >
			<template v-for="(checkbox, j) in templateCheckboxes"  #[slot(checkbox)]="{ item }" >
				<div class="d-flex justify-content-center">
					<v-checkbox v-model="item[checkbox]" @change="setCheckbox(block.tablename, item)"></v-checkbox>
				</div>
			</template>
			...
			<template #item.actions="{ item }">
				<div class="d-flex justify-content-end">
					...
					<router-link v-if="!block.non_editable" class="form-link" :to="`/admin-panel/${block.tablename}/${block.parent ? `${$route.params.parent_id}/` : ''}form/${item.id}`">
						<v-btn small color="primary" class="white--text mr-2">
							<v-icon left class="">{{  block.edit_button_icon ? block.edit_button_icon : 'mdi-pencil' }}</v-icon>
							<span>{{ block.edit_button_text ? block.edit_button_text : 'Edytuj' }}</span>
						</v-btn>
					</router-link>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getRecordName: (block, item) => block.tablename == 'users' ? item.name : (block.tablename == 'mails' ? item.subject : item.title),
			deleteItem(block, item){
				let item_name = block.tablename == 'users' ? 'użytkownika' : 'przedmiot';
				if(confirm(`Na pewno chcesz usunąć trwale ${item_name} "${this.getRecordName(block, item)}" z tabeli "${block.title}"? `)) {
			...
			acceptReservation(item) {
				axios.post('/api/reservations/accept', item).then(res => {
			...
			setCheckbox(table, item) {
				...
				axios.put(`/api/${table}/edit`, item).then(res => {
					...
					if(table == 'reservations' && item.active) this.acceptReservation(item); 
				}).catch(err => {
					this.$store.commit('setSnackbar', snackbarAlerts.error)
				})
			},
